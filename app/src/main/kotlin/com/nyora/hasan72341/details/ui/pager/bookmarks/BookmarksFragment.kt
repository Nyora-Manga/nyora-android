...
import com.nyora.hasan72341.bookmarks.ui.BookmarksSelectionDecoration
...
class BookmarksFragment : BaseFragment<FragmentMangaBookmarksBinding>(),
	...
	override fun onActionItemClicked(
		...
		return when (item.itemId) {
			R.id.action_remove -> {
				val ids = selectionController?.snapshot()?.mapTo(HashSet(), String::toLong) ?: return false
			...
			R.id.action_save -> {
				viewModel.savePages(
					pageSaveHelper,
					selectionController?.snapshot()?.mapTo(HashSet(), String::toLong) ?: return false,
				)
				mode?.finish()
				true
			}
