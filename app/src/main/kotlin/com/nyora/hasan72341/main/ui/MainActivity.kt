...
import com.nyora.hasan72341.backups.ui.periodical.PeriodicalBackupService
import com.nyora.hasan72341.browser.AdListUpdateService
...
import com.nyora.hasan72341.core.os.VoiceInputContract
...
import com.nyora.hasan72341.core.ui.widgets.SlidingBottomNavigationView
...
import com.nyora.hasan72341.databinding.ActivityMainBinding
import com.nyora.hasan72341.details.service.MangaPrefetchService
import com.nyora.hasan72341.discover.ui.DiscoverFragment
import com.nyora.hasan72341.favourites.ui.container.FavouritesContainerFragment
import com.nyora.hasan72341.history.ui.HistoryListFragment
import com.nyora.hasan72341.local.ui.LocalIndexUpdateService
import com.nyora.hasan72341.local.ui.LocalStorageCleanupWorker
...
import com.nyora.hasan72341.remotelist.ui.MangaSearchMenuProvider
import com.nyora.hasan72341.search.ui.suggestion.SearchSuggestionItemCallback
import com.nyora.hasan72341.search.ui.suggestion.SearchSuggestionListenerImpl
import com.nyora.hasan72341.search.ui.suggestion.SearchSuggestionMenuProvider
import com.nyora.hasan72341.search.ui.suggestion.SearchSuggestionViewModel
import com.nyora.hasan72341.search.ui.suggestion.adapter.SearchSuggestionAdapter
...
class MainActivity : BaseActivity<ActivityMainBinding>(), AppBarOwner, BottomNavOwner,
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		if (savedInstanceState == null) {
			onFirstStart()
			intent.getIntExtra(AppRouter.KEY_TAB, -1).takeIf { it != -1 }?.let { tabId ->
				(bottomNav ?: viewBinding.navRail)?.selectedItemId = tabId
			}
		}
	...
	override fun onApplyWindowInsets(v: View, insets: WindowInsetsCompat): WindowInsetsCompat {
		...
		val floatingNavHorizontalMargin = resources.getDimensionPixelOffset(R.dimen.bottom_nav_floating_margin_horizontal)
		val floatingNavVerticalMargin = resources.getDimensionPixelOffset(R.dimen.bottom_nav_floating_margin_vertical)
		viewBinding.bottomContainer?.updateLayoutParams<MarginLayoutParams> {
			marginStart = floatingNavHorizontalMargin + barsInsets.start(v)
			marginEnd = floatingNavHorizontalMargin + barsInsets.end(v)
			bottomMargin = (floatingNavVerticalMargin * 1.5f).toInt() + barsInsets.bottom
		}
	...
	private fun onFeedCounterChanged(counter: Int) {
		navigationDelegate.setCounter(NavItem.DISCOVER, counter)
	}
	...
	private fun adjustAppbar(topFragment: Fragment) {
		if (topFragment is FavouritesContainerFragment || topFragment is DiscoverFragment) {
	...
	private fun adjustFabVisibility(
	        ...
	        // Always show the FAB for now to verify the new separated design
	        if (!actionModeDelegate.isActionModeStarted && !isSearchOpened) {
	                if (!fab.isVisible) {
	                        fab.show()
	                }
	        } else {
	...
	private fun updateContainerBottomMargin() {
		...
		val bottomContainer = viewBinding.bottomContainer ?: return
		val newMargin = if (bottomNavBar.isPinned && bottomNavBar.isShownOrShowing) bottomContainer.height + resources.getDimensionPixelOffset(R.dimen.bottom_nav_floating_margin_vertical) else 0
