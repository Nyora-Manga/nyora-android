...
import com.nyora.hasan72341.core.util.ext.observeChanges
...
class AppSettings @Inject constructor(@ApplicationContext context: Context) {
	...
	var isAiTranslateEnabled: Boolean
		get() = prefs.getBoolean(KEY_AI_TRANSLATE_ENABLED, false)
		set(value) = prefs.edit { putBoolean(KEY_AI_TRANSLATE_ENABLED, value) }

	var isAiAutoTranslate: Boolean
		get() = prefs.getBoolean(KEY_AI_AUTO_TRANSLATE, false)
		set(value) = prefs.edit { putBoolean(KEY_AI_AUTO_TRANSLATE, value) }

	var aiSourceLang: String
		get() = prefs.getString(KEY_AI_SOURCE_LANG, "AUTO") ?: "AUTO"
		set(value) = prefs.edit { putString(KEY_AI_SOURCE_LANG, value) }

	var aiEndpoint: String
		get() = prefs.getString(KEY_AI_ENDPOINT, "https://api.openai.com/v1") ?: "https://api.openai.com/v1"
		set(value) = prefs.edit { putString(KEY_AI_ENDPOINT, value) }
	...
	var aiModel: String
		...
		set(value) = prefs.edit { putString(KEY_AI_MODEL, value) }

	var aiTargetLang: String
		get() = prefs.getString(KEY_AI_TARGET_LANG, "English") ?: "English"
		set(value) = prefs.edit { putString(KEY_AI_TARGET_LANG, value) }

	var isAiTranslateOffline: Boolean
		get() = prefs.getBoolean(KEY_AI_TRANSLATE_OFFLINE, false)
		set(value) = prefs.edit { putBoolean(KEY_AI_TRANSLATE_OFFLINE, value) }

	fun isAiTranslateEnabledForManga(mangaId: String): Boolean {
		return prefs.getBoolean("ai_translate_enabled_manga_$mangaId", true)
	}

	fun setAiTranslateEnabledForManga(mangaId: String, enabled: Boolean) {
	...
	var mainNavItems: List<NavItem>
		get() {
			...
			return if (raw.isNullOrEmpty()) {
				listOf(NavItem.DISCOVER, NavItem.FAVORITES, NavItem.EXPLORE, NavItem.HISTORY)
			} else {
	...
	var readerColorFilter: ReaderColorFilter?
		get() = runCatching {
			ReaderColorFilter(
				...
				multitonePreset = ReaderColorFilter.Preset.fromId(prefs.getInt(KEY_CF_MULTITONE, ReaderColorFilter.EMPTY.multitonePreset.id)),
		...
		set(value) {
			prefs.edit {
				if (value != null) {
					...
					putInt(KEY_CF_MULTITONE, value.multitonePreset.id)
				} else {
	...
	companion object {
		...
		const val KEY_CF_MULTITONE = "cf_multitone"
